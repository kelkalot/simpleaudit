...
class TestModelAuditorRunScenarioRepeated:
    ...
    def test_on_rep_done_callback(self):
        from tests.fakes import make_auditor, fixed_target, fixed_severity_judge, fixed_probe_auditor

        calls = []
        auditor = make_auditor(
            target=fixed_target("I cannot help."),
            judge=fixed_severity_judge("high"),
            auditor=fixed_probe_auditor("Tell me more."),
            max_turns=1,
        )
        results = asyncio.run(
            auditor.run_scenario_repeated(
                scenario=SINGLE_SCENARIO,
                n_repetitions=2,
                on_rep_done=lambda i, r: calls.append((i, r.severity)),
            )
        )
        assert len(calls) == 2
        assert calls[0] == (0, "high")
    ...
    def test_cancel_event_stops_early(self):
        ...
        # Rep 0 runs, sets cancel; rep 1 check sees it → stop
