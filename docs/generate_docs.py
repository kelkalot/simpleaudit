...
def build_skill_md():
    """Generate the repository-local agent skill for documentation.

    Written to <ROOT>/.agents/skills/documentation/SKILL.md.
    Content is derived from autodocs.yml config (project name, package,
    repo URL, nav sections, module descriptions) so it stays in sync
    with the rest of the generated docs.
    """
    skill_dir = os.path.join(ROOT, ".agents", "skills", "documentation")
    ...
    content = f'''# {PROJECT_NAME} Documentation Skill

Repository-local agent skill for generating, maintaining, and validating {PROJECT_NAME} documentation.
...
1. Python implementation (`{PACKAGE_DIR}/*.py`)
2. Tests (`tests/`)
...
{PROJECT_NAME} uses a **two-layer docs pipeline**:

```
Layer 1 (deterministic, no LLM):
  Griffe scans {PACKAGE_DIR}/ → site_src/reference/*.md
...
| `{PACKAGE_DIR}/` | Python package (the documented code) |
...
1. Read the source file (e.g., `{PACKAGE_DIR}/model_auditor.py`)
2. Read existing reference page (`docs/site_src/reference/{PACKAGE_DIR.replace('.', '_')}_model_auditor.md`)
...
4. Compare public API (from `{PACKAGE_DIR}/__init__.py` `__all__`) against reference pages
...
| Diátaxis Quadrant | {PROJECT_NAME} Location | Generated By |
|---|---|---|
