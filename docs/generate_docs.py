...
def build_mkdocs_yml(nav):
    """Write mkdocs.yml with Material theme + mkdocstrings."""
    nav_yaml = _nav_to_yaml(nav)
    content = f'''# MkDocs configuration for {PROJECT_NAME}
# Generated by docs/generate_docs.py — DO NOT EDIT by hand.
#
#   - site_name, site_description, site_url, repo_url, repo_name
#     come from docs/autodocs.yml (edit that file instead).
#   - theme, plugins, markdown_extensions: safe to edit here,
#     but they will be overwritten on the next generate_docs.py run.
#   - nav: fully regenerated on each run.
#
# To change site identity, edit docs/autodocs.yml.
